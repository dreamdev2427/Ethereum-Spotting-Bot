...
const generateTokenAlertMessage = async (tokenInfo, pairInfo, lpStatus, socials, safety) => {
    ...
    🔖Tax: Buy ${isEmpty(taxInfo?.buy)? 0: taxInfo?.buy} %, Sell ${isEmpty(taxInfo?.sell)? 0: taxInfo?.sell} % ${isEmpty(taxInfo?.sell)? 0: (taxInfo?.buy >= 0 || taxInfo?.sell >= 10) ? "🔴 High taxes - Project Likely to fail Not recommended." : "🟢 Normal"}
